...
class MetalRenderer: ObservableObject {
    ...
    private func applyComputeShader(
        ...
        if !parameters.isEmpty {
            let parameterData = try createParameterBuffer(parameters: parameters)
            computeEncoder.setBuffer(parameterData, offset: 0, index: 0)
        }
    ...
    private func createParameterBuffer(parameters: [String: Any]) throws -> MTLBuffer {
        // Простая реализация - можно расширить для разных типов параметров
        var floatParams: [Float] = []
        
        for (_, value) in parameters {
            if let floatValue = value as? Float {
                floatParams.append(floatValue)
            } else if let intValue = value as? Int {
                floatParams.append(Float(intValue))
            } else if let doubleValue = value as? Double {
                floatParams.append(Float(doubleValue))
            }
        }
        
        guard let buffer = device.makeBuffer(
            bytes: floatParams,
            length: floatParams.count * MemoryLayout<Float>.size,
