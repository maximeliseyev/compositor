...
class TextureManager {
    ...
    func forceCleanup() {
        ...
        print("🧽 Force cleanup: removed \(totalTextures) textures from pool")
    }
    
    // MARK: - Statistics
    
    func getStatistics() -> TextureManagerStats {
        let poolCount = texturePool.values.reduce(0) { $0 + $1.count }
        return TextureManagerStats(
            totalCreated: totalCreated,
            totalReused: totalReused,
            currentlyInUse: usedTextures.count,
            currentlyInPool: poolCount,
            reuseRatio: totalReused > 0 ? Double(totalReused) / Double(totalCreated + totalReused) : 0.0
        )
    }
